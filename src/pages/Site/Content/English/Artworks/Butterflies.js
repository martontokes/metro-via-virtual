...
export default function Butterflies({swiper}) {

    const [butterfliesLaunched, setButterfliesLaunched] = useState(false);
    ...
    return (
      ...
      <>
          ...
          <div id="artwork_page">
            ...
            <button onClick={() => {launchButterflies(true)}}>Watch Butterflies</button>
            { butterfliesLaunched ? <ButterfliesPlayer props={setButterfliesLaunched} /> : null }
          </div>
     </>
...
function ButterfliesPlayer({setButterfliesLaunched}) {

  const [hasLoaded, setLoaded] = useState(false);

  function onIframeLoad() {
    document.getElementById("loaderContainer").remove;
    document.getElementById("butterfliesiframe"); //make visible//
    setLoaded(true);
  }
  ...
  return (
    <>
    <Loader />
    <iframe id="butterfliesiframe" onLoad={onIframeLoad} src="/butterflies/index.html" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe>
    <button onClick={setButterfliesLaunched(false)}></button>
    </>
  )
